...
class APSExamPaperSection(models.Model):
    ...
    def _unique_copy_source_key(self):
        """Return a source key that is unique within this import.

        ``source_key`` is an internal key generated by detection and is not
        the right value to copy verbatim when a teacher duplicates a section.
        Keep the copied regions and review settings, but give the new section
        its own stable key so it can be edited as a manually added section.
        """
        self.ensure_one()
        base_key = (self.source_key or 'manual').strip() or 'manual'
        candidate = '%s_copy' % base_key
        suffix = 2
        while self.search([
            ('import_id', '=', self.import_id.id),
            ('source_key', '=', candidate),
        ], limit=1):
            candidate = '%s_copy%s' % (base_key, suffix)
            suffix += 1
        return candidate

    def copy(self, default=None):
        """Duplicate a section as a new, editable manual section."""
        defaults = dict(default or {})
        defaults.setdefault('source_key', self._unique_copy_source_key())
        defaults.setdefault(
            'sequence', max(self.import_id.section_ids.mapped('sequence') or [0]) + 1,
        )
        # Generated resources and OCR output belong to the original section.
        # A duplicate must be rebuilt independently.
        defaults.update({
            'resource_id': False,
            'resource_key': False,
            'question_html': False,
            'answer_html': False,
            'image_update_log': False,
            'ocr_state': 'pending',
            'ocr_model_id': False,
            'ocr_error': False,
        })
        duplicate = super().copy(defaults)
        if duplicate.import_id.state == 'completed':
            duplicate.import_id.write({
                'state': 'analysing',
                'progress': 60,
                'completed_at': False,
            })
    ...
    def remove_region_editor_region(self, document_type, index):
        """Remove one section image region without deleting its rendered page."""
        self.ensure_one()
        field_name = 'question_regions' if document_type == 'question' else 'answer_regions' if document_type == 'mark_scheme' else False
        if not field_name:
            raise ValidationError(_('Invalid exam document type.'))
        try:
            index = int(index)
        ...
        values = {
            field_name: regions,
            'question_pages': self._region_page_string(
                regions if document_type == 'question' else (self.question_regions or [])
            ),
            'answer_pages': self._region_page_string(
                regions if document_type == 'mark_scheme' else (self.answer_regions or [])
            ),
        }
        self.write(values)
        return True
