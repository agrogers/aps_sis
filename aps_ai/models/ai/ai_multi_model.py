...
class APSAIMultiModelFeedback(models.Model):
    ...
    def generate_multi_model_feedback(self, record, ai_run=None):
        ...
        model_ids_to_run = (
            resource.ai_model_ids.ids
            if resource and hasattr(resource, 'ai_model_ids') and resource.ai_model_ids
            else []
        )
        ...
        resource = self._get_resource_for_record(record)
        merge_responses = bool(resource.ai_merge_responses) if resource and hasattr(resource, 'ai_merge_responses') else False
        merge_response_chunks = bool(resource.ai_merge_response_chunks) if resource and hasattr(resource, 'ai_merge_response_chunks') else False
